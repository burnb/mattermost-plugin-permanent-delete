...
var (
	...
	preferenceColumnsWithoutDefault = []string{"userid", "category", "name", "value"}
	preferenceColumnsWithDefault    = []string{}
	preferencePrimaryKeyColumns     = []string{"userid", "category", "name"}
)
...
func Preferences(mods ...qm.QueryMod) preferenceQuery {
	...
	return preferenceQuery{NewQuery(mods...)}
}

// FindPreference retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindPreference(ctx context.Context, exec boil.ContextExecutor, userid string, category string, name string, selectCols ...string) (*Preference, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"preferences\" where \"userid\"=$1 AND \"category\"=$2 AND \"name\"=$3", sel,
	)

	q := queries.Raw(query, userid, category, name)
...
func (o *Preference) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"preferences\" WHERE \"userid\"=$1 AND \"category\"=$2 AND \"name\"=$3"
...
func (o *Preference) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindPreference(ctx, exec, o.Userid, o.Category, o.Name)
	if err != nil {
		return err
	}
...
func PreferenceExists(ctx context.Context, exec boil.ContextExecutor, userid string, category string, name string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"preferences\" where \"userid\"=$1 AND \"category\"=$2 AND \"name\"=$3 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, userid, category, name)
	}
	row := exec.QueryRowContext(ctx, sql, userid, category, name)
